...
export interface DeletePagesToolProps {
  ...
  /** Callback fired with the resulting blob and original file when delete succeeds */
 ...
 */
export function DeletePagesTool({ className = '', initialFile, hideUploader, onComplete }: DeletePagesToolProps) {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      if (output.success && output.result) {
        ...
        setResult(blob);
        setStatus('complete');
        if (onComplete) onComplete(blob, file);
      } else {
  ...
  }, [file, selectedPages, totalPages, onComplete]);
