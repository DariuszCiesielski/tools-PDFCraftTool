...
export type LoginFormMode = 'signin' | 'signup' | 'forgot';
...
export function LoginForm({
  ...
  const switchMode = (newMode: LoginFormMode) => {
    setEmail('');
    setPassword('');
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      {errorMessage && (
        ...
        >
          <AlertCircle
            className="w-4 h-4 mt-0.5 flex-shrink-0 text-[hsl(var(--color-destructive))]"
            aria-hidden="true"
          />
...
export function getLoginFormTitleKey(mode: LoginFormMode): string {
  return mode === 'signin'
    ? 'auth.signinTitle'
    : mode === 'signup'
      ? 'auth.signupTitle'
